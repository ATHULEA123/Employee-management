...
function asd() {
  ...
  viewImg.src = `http://localhost:5000/uploads/${id}.png`;

  fetch(`http://localhost:5000/uploads/${id}.png`, {
    method: "GET",
    headers: {
  ...
  fetch(`http://localhost:5000/api/employees/${id}`, {
    ...
    .then((data) => {
      ...
      // vieweDetailImg.src =`http://localhost:5000/api/employees/${id}/avatar`;
      let userName =
      ...
      document.getElementById("view-qualifications").innerHTML =
        data.qualifications;
      ...
      function viewage(dob) {
        ...
        if (
          ...
          (currentDate.getMonth() === month - 1 && currentDate.getDate() < date)
        ) {
          calcage--;
        }
        return calcage;
...
function vieweditformopen() {
  ...
  fetch(`http://localhost:5000/api/employees/${id}`, {
    ...
    .then((data) => {
      ...
      editviewimage.src = `http://localhost:5000/uploads/${id}.png`;
...
function dele() {
  ...
  del.addEventListener("click", function conformDelete() {
    fetch(`http://localhost:5000/api/employees/${personalId}`, {
      method: "DELETE",
    })
      .then((response) => response.json())
    ...
    window.location.href = "http://localhost:5000/api/employees";
  });
}
